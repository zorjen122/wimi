...
MessageLink::MessageLink(Node node, std::string gatewayId,
...
bool MessageLink::Healthy() const {
  return healthy.load(std::memory_order_acquire);
}

std::size_t MessageLink::Inflight() const {
  return inflight.load(std::memory_order_relaxed);
}

void MessageLink::IncrementInflight() {
...
const std::string &MessageLink::Id() const {
  return node.id;
}
...
int64_t MessageLink::LastReadAt() const {
