...
void ClientForwardService::SetGatewayStreamService(
    rpc::GatewayStreamService *service) {
  gatewayStreamService.store(service, std::memory_order_release);
}

bool ClientForwardService::ForwardToGateway(
    int64_t uid, const std::string &packet, uint32_t protocolId,
  ...
  forward.set_forward_id(std::to_string(protocolId) + ":" +
                         std::to_string(forwardId) + ":" +
                         std::to_string(uid));
