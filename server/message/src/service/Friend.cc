...
FriendService::FriendService(ClientForwardService& clientForwardService) : clientForwardService(clientForwardService) {}
...
int FriendService::StoreNotifyAddFriend(TcpPacket& request)
{
    ...
    db::FriendApply::Ptr friendApply(new db::FriendApply(from, to, db::FriendApply::Status::Wait, requestMessage));

    int ret = db::MysqlDao::GetInstance()->insertFriendApply(friendApply);
    return ret;
}

TcpPacket FriendService::NotifyAddFriend(unsigned int msgID, TcpPacket& request)
{
    TcpPacket rsp;
    ...
    rsp.set_uid(to);
    if (ret == -1) {
        LOG_INFO(businessLogger, "StoreageNotifyAddFriend save service failed, from-{}, to-{}", from, to);
    ...
    if (clientForwardService.ForwardToGateway(to, SerializeTcpPacket(senderRsp), ID_NOTIFY_ADD_FRIEND_REQ)) {
...
TcpPacket FriendService::ReplyAddFriend(unsigned int msgID, TcpPacket& request)
{
    TcpPacket rsp;
    ...
    if (StoreReplyAddFriend(request) == -1) {
        LOG_ERROR(businessLogger, "StoreageReplyAddFriend is failed, from {}, to {}", from, to);
        rsp.set_error(ErrorCodes::MysqlFailed);
    ...
    senderRsp.set_seq(db::RedisDao::GetInstance()->generateMsgId());
    if (clientForwardService.ForwardToGateway(to, SerializeTcpPacket(senderRsp), ID_REPLY_ADD_FRIEND_REQ)) {
        rsp.set_error(ErrorCodes::Success);
        return rsp;
    }
...
int FriendService::StoreReplyAddFriend(TcpPacket& request)
{
    ...
    if (accept) {
        ...
        db::Friend::Ptr friendData(new db::Friend(friendshipId, conversationId, from, to, time));

        rt = db::MysqlDao::GetInstance()->insertFriend(friendData);
        if (rt == -1) {
            LOG_ERROR(businessLogger, "insertFriend filed is failed, from {}, to {}", from, to);
            return -1;
        }
    } else {
    ...
    db::FriendApply::Ptr friendApply(new db::FriendApply(from, to, status, replyMessage, time));
    rt = db::MysqlDao::GetInstance()->updateFriendApplyStatus(friendApply);
    if (rt == -1) {
        LOG_ERROR(businessLogger, "updateFriendApplyStatus filed is failed, from {}, to {}", from, to);
    }
    // 暂用
    return rt == -1 ? -1 : friendshipId;
}

TcpPacket FriendService::PullFriendApplyList(uint32_t msgID, TcpPacket& request)
{
    TcpPacket rsp;

    int64_t uid = request.uid();
    db::FriendApply::FriendApplyGroup applyList = db::MysqlDao::GetInstance()->getFriendApplyList(uid);
...
TcpPacket FriendService::PullFriendList(uint32_t msgID, TcpPacket& request)
{
    ...
    int64_t uid = request.uid();
    db::Friend::FriendGroup friendList = db::MysqlDao::GetInstance()->getFriendList(uid);
    ...
    for (auto friendObject : *friendList) {
        ...
        db::UserInfo::Ptr friendInfo = db::MysqlDao::GetInstance()->getUserInfo(friendUid);
